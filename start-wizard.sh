...
if [ "$do_verify" != "Y" ] && [ "$do_verify" != "y" ]; then
    ...
    if [ "$(collected_val "SYNC_MODE")" = "fast" ]; then
        ...
        if [ "$ENV_NAME" = "mainnet" ]; then
            _default_rpc="https://erpc.xinfin.network"
        elif [ "$ENV_NAME" = "testnet" ]; then
            _default_rpc="https://earpc.apothem.network"
        fi
        _pivot_script="$REPO_ROOT/tools/get_pivot_for_fast_sync.sh"
        ...
        _pivot_out=$(RPC_URL="$_pivot_rpc" bash "$_pivot_script" "$ENV_NAME" 2>/dev/null)
        _pivot_rc=$?
