...
for (let i = 1; i <= config.num_machines; i++) {
  const subnet_nodes = gen_compose.genSubnetNodes(machine_id = i, num = num_per_machine[i - 1], start_num = start_num)
...
const genesis_input = gen_other.genGenesisInputFile(config.network_name, config.network_id, config.num_subnet, keys)
