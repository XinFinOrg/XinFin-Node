...
# 2. Parse genesis.json (default to genesis.json if no argument provided)
...
# Extract epoch and switchEpoch using jq
epoch=$(jq -r '.config.XDPoS.epoch' "$GENESIS_FILE")
switchEpoch=$(jq -r '.config.XDPoS.v2.SwitchEpoch' "$GENESIS_FILE")

# Validate extracted values
if [ -z "$epoch" ] || [ "$epoch" = "null" ]; then
    echo "Error: Could not parse 'epoch' from $GENESIS_FILE (path: .config.XDPoS.epoch)"
    exit 1
fi

if [ -z "$switchEpoch" ] || [ "$switchEpoch" = "null" ]; then
    echo "Error: Could not parse 'switchEpoch' from $GENESIS_FILE (path: .config.XDPoS.v2.SwitchEpoch)"
    exit 1
fi

# 3. Calculate current_epoch = floor(current_round/epoch) + switchEpoch
